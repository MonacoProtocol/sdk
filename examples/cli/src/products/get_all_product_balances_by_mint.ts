...
async function getBalances(mintPk: PublicKey) {
  ...
  const uniqueEscrowAccounts = [...new Set(response.data.productAccounts.map(
    product => product.account.commissionEscrow.toBase58()))]
  log(`${'*'.repeat(75)}`)
  uniqueEscrowAccounts.map(async (escrowAccount) => {
    try {
      ...
      log(`Products: ${response.data.productAccounts.filter((product) => product.account.commissionEscrow.toBase58() === escrowAccount).map(
        (product) => product.account.productTitle).join(', ')}`)
...
const args = getProcessArgs(["mintPk"], "npm run getProductBalancesByMint");
