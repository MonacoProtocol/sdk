...
async function getMarketPricesByPk(marketPk: PublicKey) {
  ...
  response.data.marketPrices = parseEmptyQueueItemsFromMarketPrices(response.data.marketPrices)
  
  // remove price ladder from response as they contain 317 items, comment out line below to see full ladder
  response.data.marketOutcomeAccounts.forEach((outcome) => { outcome.account.priceLadder = [] })
