...
const getAllSubCategories = async (categoryCode: string) => {
    ...
    const subCategories = await Subcategories.subcategoryQuery(connection).filterByCategory(categoryPda).fetch();
    logJson(subCategories)
}

const args = getProcessArgs(['categoryCode'], 'npm run getSubcategoriesForCategory')
